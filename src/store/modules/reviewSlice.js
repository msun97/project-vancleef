...
const initialState = {
  ...
  myreviews: JSON.parse(localStorage.getItem('currentUser')).myreviews || [],
...
export const reviewSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentProduct: (state, action) => {
      ...
      // 총 리뷰 개수 업데이트
      state.totalReviews = countTotalReviewsForProduct(
        state.reviews,
        action.payload,
      );
    ...
    addReview: (state, action) => {
      ...
      const existingReviewIndex = state.myreviews.findIndex(
      ...
      if (currentUser && currentUser.myreviews) {
        ...
        if (
          ...
          (!users[userIndex].myreviews ||
            users[userIndex].myreviews.length !== currentUser.myreviews.length)
        ) {

  ...
  const { rowCount, splitData } = useMemo(() => {
    ...
    if (totalItems >= 9) {
      ...
      return {
        rowCount: 3,
        splitData: [
          data.slice(0, itemsPerRow),
          data.slice(itemsPerRow, itemsPerRow * 2),
          data.slice(itemsPerRow * 2)
        ]
      };
  ...
  return (
    ...
    >
      {splitData.map((rowData, rowIndex) => (
        <div key={`row-${rowIndex}`} className={`w-fit ${rowIndex % 2 !== 0 ? 'mt-[-100px]' : ''}`}>
          <Swiper
            direction={'vertical'}
            slidesPerView={3}
            centeredSlides={true}

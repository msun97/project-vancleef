...
const ValueSecTwo = () => {
  ...
  useEffect(() => {
    ...
    gsap.utils
      .toArray(col1Ref.current.querySelectorAll('figure:not(:first-child)'))
      .forEach(figure => {
        tl.to(
          figure,
          {
    ...
    gsap.utils
      .toArray(col2Ref.current.querySelectorAll('figure:not(:last-child)'))
      .forEach(figure => {
        tl.to(
          figure,
          {
    ...
    gsap.utils
      .toArray(col3Ref.current.querySelectorAll('figure:not(:first-child)'))
      .forEach(figure => {
        tl.to(
          figure,
          {
    ...
    gsap.utils
      .toArray(col4Ref.current.querySelectorAll('figure:not(:last-child)'))
      .forEach(figure => {
        tl.to(
          figure,
          {
    ...
    gsap.utils
      .toArray(col5Ref.current.querySelectorAll('figure:not(:first-child)'))
      .forEach(figure => {
        tl.to(
          figure,
          {
      ...
      >
        {images.map((src, index) => (
          <figure
            key={index}
            className={`mb-4 ${
              isUpward && index !== images.length - 1 ? 'mb-32' : ''
            } ${!isUpward && index !== 0 ? 'mt-32' : ''}`}
            style={{
              marginBottom:
                isUpward && index !== images.length - 1 ? '320px' : '',
      ...
      <div className="content-page step2 w-full h-full overflow-hidden relative">
        ...
        <div className="text-wrap absolute top-1/6 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center w-full max-w-3xl px-4 z-20 ">
          ...
          >
            ...
            플래티넘과 다이아몬드에 대한 깊은 애정은{' '}
            <br className="hidden md:block" />
            "화이트 주얼리"로 알려진 탁월한 유연함을 지닌 매혹적인 피스를
            탄생으로 연결됐습니다.
          </p>
          <p
            ref={addToTextRefs}
            className="desc opacity-0 invisible text-lg md:text-xl lg:text-[32px] text-white mb-8 leading-relaxed font-bold absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 whitespace-nowrap"
          >
            르네 퓌상과 르네 심 라카즈는 반클리프 아펠만의 독특한 스타일을
            구축합니다.
          </p>
          <p
            ref={addToTextRefs}
        ...
        <div
          className="bg absolute inset-0 bg-black bg-opacity-20 z-0"

...
const Section4 = () => {
    ...
    useEffect(() => {
        ...
        const setupSlides = () => {
            Item.forEach((_, index) => {
                const slide = document.querySelector(`.swiper-slide[data-swiper-slide-index="${index}"]`);
                if (!slide) return;
        ...
        return (
            <div className="custom-pagination absolute right-10 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
                {Item.map((_, index) => (
                    <span 
                        key={index}
                        className={`w-3 h-3 rounded-full cursor-pointer ${index === activeIndex ? 'bg-gray-90' : 'bg-gray-40'}`}
    ...
    return (
        <div id="section4" ref={sectionRef} className='bg-gray-0 px-[330px] h-[100vh] relative py-[50px] overflow-hidden'>
                        ...
                        <SwiperSlide key={item.id} data-swiper-slide-index={index}>
                            <div className='flex gap-5 h-full justify-center items-center'>
                                <div className='w-fit h-fit relative main-product-container'>
                                    ...
                                    <img src={item.maindata.objectimage[1]} className='w-[500px] h-[500px] rounded-[20px] main-image' />
                                    ...
                                    <div className='absolute top-[50%] left-[50%] flex flex-col text-gray-0 -translate-x-2/4 -translate-y-2/4 w-full items-center gap-5 z-[60] text-content'>
                                        <h3 className='text-[20px]'>제품을 넘은 가치를 만나보세요</h3>
                                        <div className='flex gap-3 items-center'>
                                            <div className='border-2 border-gray-0 hover:bg-gray-0 hover:bg-opacity-20 transition-all cursor-pointer'>
                                                <img src="/icons/arrow-up-right.svg" className='w-8 h-8' />
                                            </div>
                                            <Link to='/productlist'>
                                                <p className='font-bold underline hover:text-gray-10 transition-all'>More Info</p>
                                            </Link>
                                        </div>
                                    </div>
                                </div>

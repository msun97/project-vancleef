...
const CCInquiry = ({ handleClick }) => {
  ...
  const inquiryList = user ? user.ccInquiries : [];
  ...
  const FilterButton = ({ days, label }) => {
    ...
    return (
      <li
        className={`w-[78px] h-10 border ${
          isActive ? '' : 'border-gray-20 text-gray-20'
  ...
  useEffect(() => {
    dispatch(paginationActions.addData({ pageId: 'inquiry', data: nowData }));
  }, [nowData]);
        ...
        <div className="calendar flex gap-[10px] items-center">
          <input
            ...
            className={`w-[188px] h-10 bg-gray-10 p-4 rounded-[5px] ${
              activeFilter === 'custom' ? 'border-blue-500' : ''
            }`}
        ...
        <tbody className="w-full">
          {currentPost && currentPost.length > 0 ? (
            currentPost.map(item => (
              <tr
                key={item.id}
                className="border-y border-gray-20 hover:bg-gray-10 cursor-pointer"
                onClick={() => onGo(item.tag)}
              >
                <td className="text-content-m py-5 w-[80%] text-center">
                  <div className="flex gap-4">
                    <div className="text-gray-50">[{item.tag}]</div>
                    {item.title}
                  </div>
                </td>
                <td className="text-content-m text-center py-5">{item.date}</td>
              </tr>
            ))
          ) : (
            <tr>

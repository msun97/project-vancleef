  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= ratingValue) {
        // 채워진 별
        stars.push(
          <svg key={i} width="14" height="14" viewBox="0 0 48 49" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
  ...
  return (
    <div className="w-full h-[300px] py-[40px] flex gap-[30px] border-b-2">
      <div className="flex flex-col gap-[34px]">
        ...
        <div className="bg-gray-10 w-[170px] h-[163px] rounded-tl-[180px] rounded-tr-[180px] flex justify-center items-baseline overflow-hidden">
          <img src={productImage} alt="제품 이미지" className="w-[162px] h-[162px]" />
        </div>
      </div>
      <div className="w-full relative flex flex-col gap-[15px] text-xl">
        <div className="absolute top-0 right-0 flex flex-col items-end justify-between h-full">
          ...
          {/* 내 리뷰일 경우 표시 */}
          {isMyReview && <span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded mb-1">내 리뷰</span>}
        </div>
        ...
        {images && images.length > 0 && (
          <ul className="flex gap-[6px] mt-2">
            {images.map((image, index) => (
              <li key={index}>
                <img src={image} alt={`리뷰이미지${index + 1}`} className="h-[94px] object-cover rounded-md" />
              </li>
            ))}
          </ul>
        )}

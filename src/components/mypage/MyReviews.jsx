...
const MyReviews = () => {
  ...
  const { myreviews } = user ? user : []; // 변경된 부분: Redux state에서 myreviews를 가져옴
  const myReviews = myreviews ? myreviews : [];
    ...
    for (const review of reviews) {
      if (review.productId === productId) {
        ...
        const categoryData = productdata.find(
          item => item.category === review.category,
        );
        if (categoryData) {
          const product = categoryData.data.find(
  ...
  return (
    <div>
      {myReviews.length > 0 ? (
        ...
        <p className="text-center text-gray-500 py-8 border-gray-200 text-[12px]">
          작성한 리뷰가 없습니다.
        </p>
      )}
